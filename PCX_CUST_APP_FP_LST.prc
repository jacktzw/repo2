    ...
    V_SQL := ' SELECT * FROM
          (SELECT FP.ID 产品ID,
           ...
           CASE WHEN YY.YYZT=2 AND YY.CZYYTJ=4
                     ...
                     AND (SELECT COUNT(1) FROM TCUST_APP_KHQYLS BD WHERE BD.CPYYID=YY.ID and zt=1)=0
           ...
           (SELECT SP.FJH FROM TSPXXB SP WHERE SP.ID=YY.GLSP) 双录房间号,
           (SELECT SP.ANYCHATID FROM TSPXXB SP WHERE SP.ID=YY.GLSP) ANYCHATID,
           CASE WHEN (SELECT COUNT(1) FROM TCUST_APP_KHQYLS BD WHERE BD.CPYYID=YY.ID and zt=1)=0 THEN 0 ELSE 1 END 是否查看合同
       FROM TFP_CPDM FP ,TCPYY YY,TXSXM XM
       WHERE FP.ID=YY.CPID AND YY.XSXM=XM.ID AND YY.YYZT<>-1
       AND (FP.CPZT=1 /* 20190319 YANGPEI 产品未成立即显示在募集中*/
       OR  NOT EXISTS(SELECT 1 from THTXX HT WHERE HT.CPYYID=YY.ID ) and FP.id<>29853 )  '
       ||V_WHERE ||')';
